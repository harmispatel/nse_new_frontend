...
const AdminNifty = () => {
    ...
    fetch(SETTINGS_API)
      ...
      .then((data) => {
        data?.data?.map((val) => {
          ...
          } else if (val.option === "NIFTY FUTURE") {
  ...
  const buyFutureHandle = async (type) => {
    const lot = prompt(`NIFTY FUTURE ${type} with ${FutureProfit} profit and ${FutureLoss} loss \nPlease enter lots!`)
      ...
      <div className="container">
        ...
        >
          ...
          <div className="col-md-5 float-left" style={{ float: "left" }}>
            {opFut?.op_fut_nifty &&
              <>
                <label style={{marginLeft: "-35px"}}>Current Buying: </label><Button onClick={e => opFutHandle(opFut.op_fut_nifty)} style={{ backgroundColor: opFut.live_nifty === true ? "#72ED90 " : "white", }}>{opFut.op_fut_nifty}</Button>
